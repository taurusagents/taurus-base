...
async function handleAction(input) {
  ...
  if (action === 'close') {
    try { execSync('pkill -f "chrome.*remote-debugging"', { stdio: 'ignore' }); } catch { }
    clearState();
...
function formatAXNodes(nodes) {
  ...
  for (const n of nodes) {
    ...
    if (n.childIds) childMap.set(n.nodeId, n.childIds);
  }
  ...
  function render(nodeId, depth) {
    ...
    const skip = ['none', 'generic', 'InlineTextBox', 'LineBreak'].includes(role) || (role === 'StaticText' && children.length === 0);
    const lines = [];
    ...
    for (const childId of children) {
      const child = render(childId, skip ? depth : depth + 1);
      if (child) lines.push(child);
    }
    return lines.join('\n');
  }
  const root = nodes[0]?.nodeId;
  return root ? render(root, 0) : '(empty page)';
}

const raw = process.argv[2];
if (!raw) {
  console.error('Usage: browser-cli.mjs <json-input>');
  process.exit(1);
}
